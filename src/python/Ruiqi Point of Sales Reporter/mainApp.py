...
storesCSV    = r'stores.csv'
settingsJSON = r'settings.json'
googleJSON   = r'Google credentials.json'

stores   = pandas.read_csv(storesCSV, index_col=1) 
settings = loadJSON(settingsJSON)

def getGoogleSheet(dateTime: datetime):
            ...
            try:
                if ftpType == 'ftp':
                    ...
                    with pysftp.Connection(host, username, password=password, cnopts=cnopts) as sftp:
                        sftp.put(path)
                print('\t'.join(['success', ftpType, code]))
                break
            except Exception as e:
                tried +=1
                print('\t'.join([f'fail({tried})', ftpType, code, str(e)]))
                
getGoogleSheet(datetime(2021, 4, 2))
